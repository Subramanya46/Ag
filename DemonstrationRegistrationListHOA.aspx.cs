...
{
    public partial class DemonstrationRegistrationListHOA : System.Web.UI.Page
    {
        ...
        */
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (IsPostBack == false)
                {
                    Common.DistrictAll(ddlDistrict, "E", "kkisanconstr");
        ...
        public void BindFarmersList()
        {
            try
            {
                ...
                int DistrictID = ddlDistrict.SelectedItem != null ? KKISAN.Utils.ConversionUtil.ConvertInt(ddlDistrict.SelectedItem.Value) : 0;
        ...
        protected void ddlDistrict_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                Common.TalukAll(ddlTaluk, Convert.ToInt32(ddlDistrict.SelectedItem.Value), "E", "kkisanconstr");
            }
            catch (Exception ex)
            {
                ExceptionHandler.ExceptionHistory(Session["UserID"] != null ? Session["UserID"].ToString() : "", Path.GetFileName(Request.Path), ex.Message, System.Reflection.MethodBase.GetCurrentMethod().Name);
        ...
        protected void ddlTaluk_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                Common.HobliAll(ddlHobli, Convert.ToInt32(ddlDistrict.SelectedItem.Value), Convert.ToInt32(ddlTaluk.SelectedItem.Value), "E", "kkisanconstr");
            }
            catch (Exception ex)
            {
                ExceptionHandler.ExceptionHistory(Session["UserID"] != null ? Session["UserID"].ToString() : "", Path.GetFileName(Request.Path), ex.Message, System.Reflection.MethodBase.GetCurrentMethod().Name);
